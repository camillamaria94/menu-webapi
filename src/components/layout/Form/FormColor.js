...
const LayoutFormColor = ({id}) => {
    const [values,setValues] = useState(initialValue);
    const history = useHistory();

    useEffect(()=>{
        if(id){
            axios.get(`http://localhost:3001/color/${id}`)
            .then((response)=>{
    ...
    function onSubmit(ev){
        ...
        const url = id ? `http://localhost:3001/color/${id}` : "http://localhost:3001/color";
...
export default LayoutFormColor